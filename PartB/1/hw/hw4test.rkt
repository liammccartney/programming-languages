...
(define tests
  (test-suite
   ...
   ; list-nth-mod test
   (check-exn exn:fail? (lambda () (list-nth-mod null 2))  "list-nth-mod null list")
   (check-exn exn:fail? (lambda () (list-nth-mod (list 1 2 3) -1)) "list-nth-mod negative number")
...
;   ; funny-number-stream test
    (check-equal? (stream-for-n-steps funny-number-stream 3) (list 1 2 3) "funny-number-stream test")
...
;   ; cycle-lists test
    ...
    (check-equal? (stream-for-n-steps (cycle-lists (list 2 3) (list "a" "b" "c")) 5) (list (cons 2 "a") (cons 3 "b") (cons 2 "c") (cons 3 "a") (cons 2 "b"))
                  "cycle-lists test")
;
;   ; vector-assoc test
    (check-equal? (vector-assoc 4 (vector (cons 2 1) (cons 3 1) (cons 4 1) (cons 5 1))) (cons 4 1) "vector-assoc test 1")
    (check-equal? (vector-assoc 4 (vector (cons 2 1) 4 (cons 3 1) (cons 4 1) (cons 5 1))) (cons 4 1) "vector-assoc test 2")
    (check-equal? (vector-assoc 4 (vector 1 2)) #f "vector-assoc test 4")
    (check-equal? (vector-assoc 4 (vector (cons 2 1) 4 (cons 3 1) (cons 5 1) (cons 5 1))) #f "vector-assoc test 3")
;
;   ; cached-assoc tests
